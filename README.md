...
**Workflow Suite of Tools** — Build and re-render extracted workflows with dedicated nodes: Workflow Extractor, Workflow Builder, Workflow Renderer, Workflow Bridge, and Workflow Model Loader. Workflow Renderer includes easy, pre-built workflow handling for SDXL, Flux 1, Flux 2, Qwen, Z-Image, Wan Image, plus Wan I2V and Wan T2V video workflows. In practice, setup is simple: configure the Workflow Builder once, then everything else is automatic. Add Workflow Renderer and connect your Save Image / Save Video node to run the full pipeline with the correct prompts, model routing, LoRA stacks, and workflow context.
___
